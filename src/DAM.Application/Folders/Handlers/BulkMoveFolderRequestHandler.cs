...
{
    public class BulkMoveFolderRequestHandler : HandlerBase<BulkMoveFolderRequest, HandlerResult<List<MoveFolderDto>>>
    {
        ...
        public BulkMoveFolderRequestHandler(ICacheProvider cacheProvider, IDbContext dbcontext, IMapper mapper, IConfiguration configuration)
        {
        ...
        public override HandlerResult<List<MoveFolderDto>> HandleRequest(BulkMoveFolderRequest request, CancellationToken cancellationToken, HandlerResult<List<MoveFolderDto>> result)
        {
            if(request.Folders.Count > 0)
            {
                request.Folders.ForEach(f =>
                {
                    var newParentFolderId = f.ParentFolderId;
