...
{
    public class GetAssetRequestHandler : HandlerBase<GetAssetRequest, HandlerResult<IEnumerable<AssetDto>>>
    {
        ...
        public GetAssetRequestHandler(ICacheProvider cacheProvider, IDbContext dbcontext, IMapper mapper, IConfiguration configuration, IAzureStorageService azureStorageService)
        {
        ...
        public override HandlerResult<IEnumerable<AssetDto>> HandleRequest(GetAssetRequest request, CancellationToken cancellationToken, HandlerResult<IEnumerable<AssetDto>> result)
        {
            ...
            {
                assets = _dbcontext.Assets.Where(a => a.Status != Convert.ToInt32(AssetStatus.Archived) && a.Status != Convert.ToInt32(AssetStatus.Deleted))
            ...
            foreach (var asset in result.Entity)
            {
                ...
                if(request.AssetId > 0)
                {
                    foreach(var ver in asset.AssetVersions)
                    {
                        ver.CreatedByName = allUsers.First(u => u.Id == ver.CreatedById).UserName;
                    }
                }
                //get active version
